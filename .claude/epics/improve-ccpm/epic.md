...
progress: 0%
prd: .claude/prds/improve-ccpm.md
...
- **Data directory**: Move epics and PRDs from `.claude/epics/` and `.claude/prds/` to `.pm/epics/` and `.pm/prds/`. The `.pm/` directory is outside `.claude/` so Claude Code won't treat edits as settings modifications.
...
Physical file moves:
- `.claude/epics/` → `.pm/epics/` (including `archived/` subdirectory)
- `.claude/prds/` → `.pm/prds/`
...
- No active epics in `.claude/epics/` that would conflict with the move (currently only archived + this one)

## Success Criteria (Technical)

- `grep -r '\.claude/epics/' .claude/ccpm/ .claude/commands/` returns zero matches after relocation
- `grep -r '\.claude/prds/' .claude/ccpm/ .claude/commands/` returns zero matches after relocation
