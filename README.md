...
- `librsvg` and `imagemagick` (only needed for `make dev` / `make build` on macOS — they generate the app icon from `appicon.svg`). Install on macOS: `brew install librsvg imagemagick`
