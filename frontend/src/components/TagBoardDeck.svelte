...
<div class="tag-board-deck">
  ...
  <!--
    Foreground board area (issue #145).

    The strip above is the canonical navigation surface. Only the
    selected board mounts the `board` snippet (AD-5 invariant
    preserved). EditTaskDialog is mounted at the EisenKanView level
    (outside this deck), so dialog state survives any board swap
    without needing content-mount gating here.
  -->
  <div class="tag-board-stack">
    <TagBoardCard
      label={effectiveTag}
      focused={focusGroup.includes(effectiveTag)}
    >
      {@render board(slice)}
    </TagBoardCard>
  </div>
...
<style>
  ...
  /*
   * Foreground board container. Plain flex column — the foreground
   * card uses `flex: 1` to consume the remaining vertical space.
   */
  .tag-board-stack {
    flex: 1;
    min-height: 0;
